...
export default function (props: any) {
  ...
  return (
    ...
    >
      <div id="chart" className="absolute rounded-md mt-10 w-[98%] mx-auto left-0 right-0 h-[92%]"></div>
    </motion.div>
  );
}
      ...
      data.data.map((d: any, index: number) => {
        ...
        if (index > 0){
          vdata.push({
            time:d[0]+32400,
            value: d[5],
            color: data.data[index-1][5] < d[5] ? 'rgba(255,82,82, 0.8)' : 'rgba(0, 150, 136, 0.8)'
          }) 
        }
        else {
          vdata.push({
