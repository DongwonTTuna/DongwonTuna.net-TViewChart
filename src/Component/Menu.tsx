...
export default function (props: any){
    ...
    return (
        <motion.div initial={{opacity: 0}} animate={{opacity:1}} transition={{duration:2}} className='backgr w-[90vw] min-w-[1200px] h-[180px] mx-auto mt-16 rounded-lg flex items-center justify-between'>
            <motion.div onClick={()=>props.setExc('binance')} className="ml-4 rounded-xl hover:bg-slate-400 p-4 transition-colors duration-500" initial={{opacity: 0}} animate={{opacity:1}} transition={{duration:1}}>
            ...
            <motion.div onClick={()=>props.setExc('ftx')} className="mr-4 rounded-xl hover:bg-slate-400 p-4 transition-colors duration-500" initial={{opacity: 0}} animate={{opacity:1}} transition={{duration:1,delay:0.9}}>
